...
class SAAttnMem(nn.Module):
    def __init__(self, input_dim, d_model, kernel_size, dropout=0):
        super().__init__()
    ...
    def forward(self, h, m):
        ...
        i = torch.sigmoid(i)
        g = torch.tanh(g)
        m_next = self.dropout2d(i * g) + (1 - i) * m
        h_next = torch.sigmoid(o) * m_next
        return h_next, m_next, Z


class SAConvLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, d_attn, kernel_size, dropout=0):
        super().__init__()
        self.input_dim = input_dim
    ...
    def forward(self, inputs, first_step=False):
        ...
        self.hidden_state, self.memory_state, Z = self.sa(self.hidden_state, self.memory_state)
        return self.hidden_state, self.hidden_state, Z


class SAConvLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, d_attn, kernel_size, device, dropout=0):
        super().__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
    ...
    def forward(self, input_x, input_frames=7, future_frames=5, output_frames=11,
        ...
        for t in range(total_steps):
            ...
            for layer_idx in range(self.num_layers):
                input_ , attention_h, attention_Z = self.layers[layer_idx](input_, first_step=first_step)
                att_h_maps.append(attention_h)
                att_Z_maps.append(attention_Z)
        ...
        return outputs, att_h_maps, att_Z_maps
